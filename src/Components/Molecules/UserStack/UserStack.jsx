...
const UserStack = ({ usersData, maxItems = 7}) => {
  ...
  const COLORS = ["#0A0E1B", "#498DD6", "#29BB77",];
  return (
    <div className="flex items-center gap-2">
      <div className="flex -space-x-4">
        <UserStackContainer className="flex">
          <UserNameChip className="w-10 h-10 font-medium uppercase rounded-full" bgColor={COLORS[0]}>
            {displayedItems && displayedItems[0][0]}
          </UserNameChip>
        </UserStackContainer>
        <UserStackContainer className="flex -space-x-8">
          {displayedItems &&
            displayedItems.map((username, index) => {
              return (
                <UserNameChip
                  className="w-10 h-10 font-medium uppercase rounded-full"
